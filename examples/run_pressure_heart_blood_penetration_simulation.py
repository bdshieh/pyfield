...
from extra import calc_path_att, bsc_to_fir, bsc_to_filt


##################################################################

transducer_module_str = 'interaction2.definitions.lockwood_vernier_element'

# bsc parameters
bsc_filepath = 'd:/data/tissue simulation/simulation/tissue_bsc_data.h5'
blood_bsc_key = 'bsc/powerfit/blood_hmtc8_shung'
heart_bsc_key = 'bsc/powerfit/heart_dog_odonnell'
...
zs = np.array([0.0, 0.001, 0.03, 0.032, 0.04, 0.042, 0.05, 0.052, 0.06, 0.062, 
    0.07, 0.072, 0.08, 0.082, 0.09, 0.092, 0.1, 0.102, 0.11, 0.112, 0.12, 0.122, 
...
for zid in xrange(1, zs.size - 1):
    ...
    layers.append(np.c_[sp.rand(nscat)*lengthx, sp.rand(nscat)*lengthy, 
        sp.rand(nscat)*lengthz] - np.array([lengthx, lengthy, lengthz])/2. + box_center)
...
def run_simulation():
    ...
    for lid, l in enumerate(layers):
        ...
        for idx, pt in tqdm(enumerate(l), total=len(l)):
            ...
            tx_sir = np.pad(tx_sir*fs, (int(round(tx_t0*fs)), 0), 'constant')
            rx_sir = np.pad(rx_sir*fs, (int(round(rx_t0*fs)), 0), 'constant')
...
if __name__ == '__main__':
    ...
    np.savez('d:/data/tissue simulation/simulation/heart_blood_penetration_phantom_data.npz', p=p, t=t, z=z, 
        p_layers=p_layers, *results)
